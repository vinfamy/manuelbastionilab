...
class ProxyEngine:
    ...
    def fit_proxy_object_each(self,proxy_offset=1.0, proxy_threshold = 0.025, create_proxy_mask = True, morph_name = "empty_morph"):
        ...
        proxy_shapekey = algorithms.new_shapekey(proxy, morph_name)
    ...
    def fit_proxy_object(self,proxy_offset=1.0, proxy_threshold = 0.025, create_proxy_mask = True):
        status, proxy, body = self.get_proxy_fitting_ingredients()
        excludes = ["Body", "Cheeks", "Chin", "Ears", "Expressions", "Eyebrows", "Eyelids", "Eyes", "Fantasy", "Mouth", "Nose", "Jaw"]
        
        for sk in body.data.shape_keys.key_blocks:
            #if "Abdomen" not in sk.name:
            if not any(x in sk.name for x in excludes):
                sk.value = 1
                self.fit_proxy_object_each(proxy_offset, proxy_threshold, create_proxy_mask, sk.name)
                sk.value = 0
                proxy.data.shape_keys.key_blocks[sk.name].value = 0
